...
class TClassDef:
    def __init__(self, tclass_source, interpreter):
        self.interpreter = interpreter
        self.name = tclass_source[1]
        self.class_source = tclass_source
        fields_and_methods_start_index = (3)
        
        self.__create_tclass_field_list(tclass_source[fields_and_methods_start_index:])
        self.__create_tmethod_list(tclass_source[fields_and_methods_start_index:])
    def get_name(self):
        return self.name
    ...
    def __create_tclass_field_list(self, tclass_body):
        ...
        for member in tclass_body:
            ...
            if member[0] == InterpreterBase.FIELD_DEF:
                ...
                var_def = self.__create_variable_def_from_tclass_field(member)
                self.fields.append(var_def)
                self.field_map[member[2]] = var_def
                fields_defined_so_far.add(member[2])

    def __create_variable_def_from_tclass_field(self, tfield_def):
        actual_value = InterpreterBase.NULL_DEF
        if len(tfield_def) > 3: 
            actual_value = tfield_def[3]
        var_def = VariableDef(
            Type(tfield_def[1]), tfield_def[2], create_value(actual_value)
        )
        if not '@' in tfield_def[1]:
            if tfield_def[1] in self.interpreter.type_manager.map_tclass_to_parameters:
                self.interpreter.error(
                ErrorType.TYPE_ERROR,
                "invalid type/type mismatch with field " + tfield_def[2],
                tfield_def[0].line_num,
            )
    ...
    def __create_tmethod_list(self, class_body):
        self.methods = []
        self.method_map = {}
        ...
        for member in class_body:
            if member[0] == InterpreterBase.METHOD_DEF:
                ...
                self.__check_tmethod_names_and_types(method_def)
    ...
    def __check_tmethod_names_and_types(self, method_def):
        if not self.interpreter.is_valid_type(
            ...
            if not('@' in method_def.return_type.type_name):
                return
...
class ClassDef:
    ...
    def __create_variable_def_from_field(self, field_def):
        ...
        if len(field_def) > 3:
            default_value = create_value(field_def[3])
        else:
            #default_value = InterpreterBase.NULL_DEF
            default_value = create_default_value(field_type)
        var_def = VariableDef(
            field_type, field_def[2], default_value
        )
        if not '@' in field_def[1]:
            if field_def[1] in self.interpreter.type_manager.map_tclass_to_parameters:
                self.interpreter.error(
                ErrorType.TYPE_ERROR,
    ...
    def __check_method_names_and_types(self, method_def):
        if not self.interpreter.is_valid_type(
            method_def.return_type.type_name,True
        ) and method_def.return_type != Type(InterpreterBase.NOTHING_DEF): #checks that return type isn't a defined type or void
            self.interpreter.error(
                ErrorType.TYPE_ERROR,
                "invalid return type for method " + method_def.method_name,
