from classv3 import VariableDef, TClassDef
import copy
...
class ObjectDef:
    ...
    def __init__(self, interpreter, class_def, anchor_object=None, trace_output=False,tclass=False):
    ...
    def __get_obj_with_method(self, start_obj, method_name, actual_params):
        cur_obj = start_obj
        if not isinstance(self.class_def,TClassDef):
    ...
    def call_method(self, method_name, actual_params, super_only, line_num_of_caller):
        ...
        elif status == ObjectDef.STATUS_THROW:
    ...
    def __execute_statement(self, env, return_type, code):
        ...
        if tok == InterpreterBase.BEGIN_DEF:
            ...
            status, result =  self.__execute_throw(env, return_type, code)
    ...
    def __execute_throw(self,env, return_type, code):
        ...
        if not isinstance(result,tuple):
            if result.type().type_name != InterpreterBase.STRING_DEF: 
                self.interpreter.error(
                    ErrorType.TYPE_ERROR,
                    "non-string thrown  " + code[1],
                )
            else:
                status = ObjectDef.STATUS_THROW
                ret_value = result
        else:
            status = result[0]
            ret_value = result[1]
    ...
    def __execute_try(self,env, return_type, code):
        ...
        return_value = None
        execute_block = code[1]
        catch_block = code[2]
        status,return_value = self.__execute_statement(env, return_type, execute_block)
        ...
        if status == ObjectDef.STATUS_THROW:
            env.block_nest()
            #execption_Variable_code = "exception "+return_value[1].value()
            var_name = "exception"
            execption_Variable_code = []
            execption_Variable_code.append(result.type().type_name)
            execption_Variable_code.append(var_name)
            value = result.value()
            if isinstance(value,str):
                value = '"'+value+'"'
            execption_Variable_code.append(value)
            exception_variables = []
            exception_variables.append(execption_Variable_code)
            self.__add_locals_to_env(env, exception_variables, code[0].line_num)
            status, return_value = self.__execute_statement(env, return_type, catch_block)
            env.block_unnest()
        return status, return_value
    ...
    def __add_locals_to_env(self, env, var_defs, line_number):
        for var_def in var_defs:
            ...
            if not '@' in var_def[0]:
                if var_def[0] in self.interpreter.type_manager.map_tclass_to_parameters:
                    self.interpreter.error(
                    ErrorType.TYPE_ERROR,
                    "invalid type/type mismatch with field " + var_name
                )
    ...
    def __execute_call(self, env, code):
        ...
        if (isinstance(result,tuple)):
            if len(result) >1:
                if result[0] == ObjectDef.STATUS_THROW:
                    status = ObjectDef.STATUS_THROW
    ...
    def __set_variable_aux(self, env, var_name, value, line_num):
        ...
        set_result = self.__set_local_or_param(env, var_name, value, line_num)
        if not isinstance(set_result, tuple):
            if set_result:  # may report a type error
                return
            set_field_result = self.__set_field(var_name, value, line_num)
            if not isinstance(set_field_result,tuple):
                if set_field_result:  # may report a type error
                    return
                self.interpreter.error(
                    ErrorType.NAME_ERROR, "unknown field/variable " + var_name, line_num
                )
            else:
                return set_field_result[0],set_field_result[1]
            self.interpreter.error(
                ErrorType.NAME_ERROR, "unknown field/variable " + var_name, line_num
            )
        else:
    ...
    def __evaluate_expression(self, env, expr, line_num_of_statement):
        ...
        if operator in self.binary_op_list:
            ...
            if (not isinstance(operand1, tuple)) and (not isinstance(operand2,tuple)):
                if (
                ...
                if isinstance(operand1, tuple):
                    if (operand1[0] == ObjectDef.STATUS_THROW):
                        status = ObjectDef.STATUS_THROW
                        result = operand1[1]
                elif isinstance(operand2, tuple):
                    if (operand2[0] == ObjectDef.STATUS_THROW):
                        status = ObjectDef.STATUS_THROW
                        result = operand2[1]
                return status , result
    ...
    def __execute_call_aux(self, env, code, line_num_of_statement):
        ...
        if obj_name == InterpreterBase.ME_DEF:
            obj = self
            obj_val = Value(Type(obj.class_def.get_name()), obj)
        ...
        for args in actual_args:
            if type(args) is tuple:
                if  args[0] == ObjectDef.STATUS_THROW:
                    return args[0], args[1]
        #need to check if obj is of primitive  type if yes, raise error.
       
        if obj_val.type().type_name in self.interpreter.type_manager.primitive_types:
            self.interpreter.error(
                ErrorType.NAME_ERROR,
                f"cannot invoke method on {obj_val.type().type_name}",
                code[0].line_num,
            )
        else:
            return_value = obj.call_method(code[2], actual_args, super_only, line_num_of_statement)
            return return_value
